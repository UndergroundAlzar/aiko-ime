    ...
    F: Fn() + Send + Sync + 'static,
{
    ...
    unsafe extern "system" fn keyboard_hook_proc(
        ...
        if code >= 0 {
            ...
            let is_key_up = event_type == WM_KEYUP || event_type == WM_SYSKEYUP;
            let is_key_down = event_type == WM_KEYDOWN || event_type == WM_SYSKEYDOWN;

            tracing::debug!(
                "keyboard_hook_proc: code={}, vk_code={}, event_type={}, is_key_up={}, is_key_down={}",
                code,
                vk_code,
                event_type,
                is_key_up,
                                ...
                                if hook_state.has_other_keys_pressed {
                                    ...
                                    if let Some(last) = hook_state.last_release {
                                        ...
                                        if elapsed <= hook_state.interval {
                                            // Double-tap detected!
                                            tracing::info!("Double-tap Ctrl detected safely!");
                                            (hook_state.callback)();
                                            hook_state.last_release = None;
                                        } else {
